...
/*   Updated: 2024/06/25 16:15:52 by fde-jesu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_pipe_sequence(t_token *token)
{
	while(token)
	{
		if ((token->type == PIPE && !token->next) ||(token->type == PIPE && !token->prev) || \
		(token->type == PIPE && (token->next && token->next->type == PIPE)))
		{
...
int check_syntax(t_shell *sh)
{
	...
	if (check_pipe_sequence(tmp))
		return 1; // error . pipe followed by another pipe
}